...
module Seismograph
  class Sensor
    ...
    def increment(description, count = 1, **params)
      Gateway.increment(stat(description), count, gateway_params(params))
    end

    def decrement(description, count = 1, **params)
      Gateway.decrement(stat(description), count, gateway_params(params))
    end
